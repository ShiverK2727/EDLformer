...
def validate(model, dataloader, config, dataset_type):
    """验证循环，适配新的指标返回格式。"""
    def semantic_inference(output, current_dataset_type):
        processed_outputs = process_edlmaskformer_outputs(
        ...
        extracted = extract_processed_outputs(processed_outputs)
        expert_mask_probs = extracted['expert_probs'].detach()
        expert_cls_probs = extracted['cls_probs'].detach()
        
        if config['model'].get('decoder_cls_target_type', 'single') == 'single':
            expert_cls_probs = expert_cls_probs[..., :-1]
        
        semseg = torch.einsum("bqn,bqchw->bnchw", expert_cls_probs, expert_mask_probs)
        
        if current_dataset_type == 'RIGA':
            # [验证转换] 将模型的3通道输出转换为2通道disc/cup格式
            semseg_disc_cup = convert_ring_cup_to_disc_cup(semseg, thresh=0.5, is_label=False)  # 预测数据使用阈值
            return semseg_disc_cup
        else:
            return semseg
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='Validating', leave=False, ncols=120, file=sys.stdout):
            ...
            preds_for_validation = semantic_inference(model(images), dataset_type)  # 获取2通道预测
