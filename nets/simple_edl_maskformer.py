...
from .maskdecoder_beta import MultiExpertsDecodeEDL
from logger import log_info
from typing import List, Optional


class SimpleEDLMaskFormer(nn.Module):
    """
    ...
    def __init__(
        ...
        self.predictor = MultiExpertsDecodeEDL(
            ...
            hidden_dim=mask_dim,
            num_queries=num_experts + num_extra_queries,
            ...
            cls_type=decoder_cls_target_type,
        )
        ...
        log_info(f"  Classification output dimension: {self.predictor.alpha_cls_embed.out_features if hasattr(self.predictor, 'alpha_cls_embed') else 'N/A'}")
        log_info(f"="*80)
...
class SimpleMaskFormerMulti(nn.Module):
    """
    基于MultiExpertsDecodeEDL的SimpleMaskFormer版本，支持多分割类别输出。
    每个query可以输出多个分割类别的mask。
    """
    def __init__(
            self,
            in_channels: int = 3,
            num_seg_classes: int = 2,  # 分割类别数
            num_cls_classes: int = 12,  # 分类类别数
            num_queries: int = 20,
            # Encoder configuration
            backbone_encoder_name: str = "resnet34",
            backbone_encoder_weights: str = "imagenet", 
        ...
        self.segmentation_head = nn.Conv2d(
            backbone_decoder_channels[-1], num_seg_classes, kernel_size=1, stride=1, padding=0
        )
        self.deep_supervision_heads = nn.ModuleList([
            nn.Conv2d(in_ch, num_seg_classes, kernel_size=1, stride=1, padding=0)
            for in_ch in predictor_in_channels
        ])
        ...
        # Initialize predictor with EDL decoder
        self.predictor = MultiExpertsDecodeEDL(
            in_channels=predictor_in_channels,
            ...
            non_object=decoder_non_object,
            mask_threshold=decoder_mask_threshold,
            cls_type='multi',  # 设置为多类输出模式
        )
        
        log_info(f"SimpleMaskFormerMulti initialized with:")
        log_info(f"  Encoder: {backbone_encoder_name} ({backbone_encoder_weights})")
        log_info(f"  Encoder channels: {encoder_channels}")
        log_info(f"  Decoder channels: {backbone_decoder_channels}")
        log_info(f"  Predictor input channels: {predictor_in_channels}")
        log_info(f"  Mask pixel indices: {mask_pixel_idxs}")
        log_info(f"  Num queries: {num_queries}")
        log_info(f"  Num seg classes: {num_seg_classes}, Num cls classes: {num_cls_classes}")
        log_info(f"="*80)

    def forward(self, input_image):
        """
        Forward pass through the complete SimpleMaskFormerMulti.
        ...
        # 1) Segmentation head on final decoder feature (B, num_seg_classes, H, W)
        pix_pred_mask = self.segmentation_head(de_feats[-1])  # (B, num_seg_classes, H, W)

        # 2) Deep supervision heads for selected decoder features
        aux_pix_pred_masks = []
        for idx, (head, mask_idx) in enumerate(zip(self.deep_supervision_heads, self.mask_pixel_idxs)):
            feat = de_feats[mask_idx]  # (B, C_in, H_i, W_i)
            ds_mask = head(feat)  # (B, num_seg_classes, H_i, W_i)
            predictions['aux_outputs'][idx]['pix_pred_masks'] = ds_mask
            aux_pix_pred_masks.append(ds_mask)
